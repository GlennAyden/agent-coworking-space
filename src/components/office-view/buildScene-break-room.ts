...
export function buildBreakRoom({
  ...
  const visibleBreakAgents = hiddenAgentIds
    ? breakAgents.filter((agent) => !hiddenAgentIds.has(agent.id))
    : breakAgents;

  if (visibleBreakAgents.length > 0) {
    const phase = Math.floor(Date.now() / 4000);
    const speakerCount = Math.min(2, visibleBreakAgents.length);
    for (let speakerIndex = 0; speakerIndex < speakerCount; speakerIndex++) {
      const speakerIdx = (phase + speakerIndex) % visibleBreakAgents.length;
      const agent = visibleBreakAgents[speakerIdx];
