...
import { type AgentTravel, updateAgentTravelAnimations } from "./agent-travel";
...
export function runOfficeTickerStep(ctx: OfficeTickerContext): void {
  ...
  const completedAgentTravel = updateAgentTravelAnimations(ctx.agentTravelAnimationsRef, tick);
  if (completedAgentTravel) ctx.onAgentTravelComplete?.();
}
