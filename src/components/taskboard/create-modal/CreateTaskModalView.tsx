...
export default function CreateTaskModalView({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto overscroll-contain bg-black/60 p-2 backdrop-blur-sm sm:items-center sm:p-4"
      onClick={(event) => {
    ...
    >
      <div
        ...
        className={`my-2 flex max-h-[calc(100dvh-1rem)] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 shadow-2xl transition-[max-width] duration-300 ease-out sm:my-0 sm:max-h-[calc(100dvh-2rem)] lg:max-h-[calc(100dvh-2rem)] lg:max-w-2xl ${
          createNewProjectMode ? "lg:max-w-5xl" : ""
        }`}
      >
        <div className="flex shrink-0 items-center justify-between border-b border-slate-700 px-5 py-3 sm:px-6 sm:py-4">
          <h2 id="create-task-modal-title" className="text-lg font-bold text-white">
        ...
        <form onSubmit={onSubmit} className="flex min-h-0 flex-1 flex-col">
          <div
            ...
            className={`min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-3 sm:px-6 sm:py-4 ${createNewProjectMode ? "lg:grid lg:grid-cols-2 lg:gap-5" : ""}`}
          >
          ...
          <div className="flex shrink-0 justify-end gap-3 border-t border-slate-700 px-5 py-3 sm:px-6 sm:py-4">
            <button
              type="button"
              onClick={onRequestClose}
