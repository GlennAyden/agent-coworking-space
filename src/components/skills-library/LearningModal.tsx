        ...
        <div className="space-y-4 overflow-y-auto px-5 py-4 max-h-[calc(90vh-72px)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {representatives.map((row) => {
              ...
              const providerAgents = agents.filter((agent) => agent.cli_provider === row.provider);
              const providerAgentNames = providerAgents.map((agent) =>
              ...
              const coverageText =
                hasAgent && providerAgentNames.length > 0
                  ? `${t({ ko: "ì ìš© ëŒ€ìƒ", en: "Covers", ja: "å¯¾è±¡", zh: "é€‚ç”¨" })} ${providerAgentNames.length}: ${providerAgentNames.join(", ")}`
                  : null;
                ...
                >
                  ...
                  <div className="relative z-10 flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {coverageText && (
                        <div className="mt-0.5 truncate text-[10px] text-slate-500" title={coverageText}>
                          {coverageText}
                        </div>
                      )}
                    </div>
