...
import type { Agent, CompanyStats, Project, Task } from "../types";
import { mapWorkflowDecisionItemsRaw } from "./decision-inbox";
import { areAgentListsEquivalent, areProjectListsEquivalent, areTaskListsEquivalent } from "./utils";
...
export function useLiveSyncScheduler({
  ...
  const runLiveSync = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([nextTasks, nextProjectPage, nextAgents, nextStats, nextDecisionItems]) => {
        setTasks((prev) => (areTaskListsEquivalent(prev, nextTasks) ? prev : nextTasks));
        setProjects((prev) => (areProjectListsEquivalent(prev, nextProjectPage) ? prev : nextProjectPage));
  ...
  }, [setAgents, setDecisionInboxItems, setProjects, setStats, setTasks, shouldIncludeSeedAgents]);
