...
export function useAppLabels({
  ...
  const loadingTitle = pickLang(uiLanguage, {
    ko: "Agent Coworking Space 로딩 중...",
    en: "Loading Agent Coworking Space...",
    ja: "Agent Coworking Spaceを読み込み中...",
    zh: "Agent Coworking Space 加载中...",
  });
  ...
  const effectiveUpdateStatus = forceUpdateBanner
    ? {
        ...
        release_url: updateStatus?.release_url ?? "https://github.com/GlennAyden/agent-coworking-space/releases/latest",
        checked_at: Date.now(),
        enabled: true,
        repo: updateStatus?.repo ?? "GlennAyden/agent-coworking-space",
        error: null,
      }
    : updateStatus;
  ...
  const updateReleaseUrl =
    ...
    `https://github.com/${effectiveUpdateStatus?.repo ?? "GlennAyden/agent-coworking-space"}/releases/latest`;
