...
export function useAppActions({
  ...
  const handleCreateTask = useCallback(
    async (input: {
      ...
      try {
        ...
        const [tks, projectPage] = await Promise.all([
          api.getTasks(),
  ...
  const refreshTasksAndAgents = useCallback(async () => {
    ...
    const [tks, projectPage, ags] = await Promise.all([
      api.getTasks(),
      api.getAllProjects().catch(() => []),
    ...
    setProjects(projectPage);
    setAgents(ags);
  }, [setProjects, setTasks, setAgents, settings.officeWorkflowPack]);
  ...
  const handleAgentsChange = useCallback(() => {
    ...
    api
      ...
      .then(setProjects)
      .catch(console.error);
  }, [setAgents, setDepartments, setProjects, setTasks, settings.officeWorkflowPack]);
