...
describe("initializeOAuthRuntime", () => {
  ...
  it("upgrades legacy provider checks to include kimi and hermes for agents and skill history", () => {
    ...
    expect(agentSql).toContain("'hermes'");
    expect(historySql).toContain("'kimi'");
    expect(historySql).toContain("'hermes'");
  });

  it("upgrades provider checks that already include kimi but not hermes", () => {
    db = new DatabaseSync(":memory:");
    applyBaseSchema(db);
    db.exec(`
      DROP TABLE agents;
      CREATE TABLE agents (
        ...
        cli_model TEXT,
        cli_reasoning_level TEXT,
        avatar_emoji TEXT NOT NULL DEFAULT 'bot',
        sprite_number INTEGER,
        personality TEXT,
        status TEXT NOT NULL DEFAULT 'idle' CHECK(status IN ('idle','working','break','offline')),
        current_task_id TEXT,
      ...
      VALUES ('legacy-kimi-agent', 'Legacy Kimi Agent', 'junior', 'kimi', 456);
      ...
      VALUES ('history-kimi', 'job-kimi', 'kimi', 'repo', 'skill', 'Skill', 'queued', 'echo test');
    `);
    ...
    const row = db.prepare("SELECT cli_provider FROM agents WHERE id = ?").get("legacy-kimi-agent") as {
      cli_provider: string;
    };

    expect(agentSql).toContain("'hermes'");
    expect(historySql).toContain("'hermes'");
    expect(row.cli_provider).toBe("kimi");
  });
