...
function resolveAgentSkillDir(agent: string): string | null {
  if (agent === "codex") {
    const sharedSkillDir = path.join(process.cwd(), ".agents", "skills");
    if (fs.existsSync(sharedSkillDir)) return sharedSkillDir;
...
function parseSkillListLinkState(rawOutput: string, candidates: string[]): SkillLinkState {
  const lines = stripAnsiControl(rawOutput)
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  for (const candidate of candidates) {
    ...
    if (!row) continue;
    if (/agents?\s*:\s*not\s+linked/i.test(row)) return "not_linked";
    if (/agents?\s*:/i.test(row)) return "linked";
    return "linked";
  }
...
async function detectSkillLinkStateFromCliList(
  execWithTimeout: RuntimeContext["execWithTimeout"],
  agent: string,
  candidates: string[],
): Promise<{ state: SkillLinkState; output: string }> {
  try {
    const output = await execWithTimeout(
      SKILLS_NPX_CMD,
      ["--yes", "skills@latest", "list", "--agent", agent],
      SKILL_LIST_TIMEOUT_MS,
    );
    return { state: parseSkillListLinkState(output || "", candidates), output: stripAnsiControl(output || "").trim() };
  } catch (err) {
    return { state: "unverifiable", output: formatExecError(err) };
  }
}
...
export function createSkillLearnCore(ctx: RuntimeContext) {
  ...
  async function runSkillUnlearnForProvider(
    ...
    const cliPreState = await detectSkillLinkStateFromCliList(execWithTimeout, agent, candidates);
    attempts.push({ skill: "__precheck_cli_list__", output: `state=${cliPreState.state}` });
    const fsPreState =
      cliPreState.state === "unverifiable" ? detectSkillLinkStateFromFilesystem(agent, candidates) : "unverifiable";
    if (fsPreState !== "unverifiable") {
      attempts.push({ skill: "__precheck_filesystem__", output: `state=${fsPreState}` });
    }
    const preState = cliPreState.state !== "unverifiable" ? cliPreState.state : fsPreState;
    ...
    const cliPostState = await detectSkillLinkStateFromCliList(execWithTimeout, agent, candidates);
    const fsPostState =
      cliPostState.state === "unverifiable" ? detectSkillLinkStateFromFilesystem(agent, candidates) : "unverifiable";
    const postState = cliPostState.state !== "unverifiable" ? cliPostState.state : fsPostState;
